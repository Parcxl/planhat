...
const orders = [
  { id: "PK-1281", name: "Studio Nova", items: "5 items", status: "Te verpakken", color: "#1A5EE5" },
  { id: "PK-1282", name: "Urban Goods", items: "3 items", status: "Te verpakken", color: "#1A5EE5" },
  { id: "PK-1283", name: "Maison Vale", items: "4 items", status: "Te verpakken", color: "#1A5EE5" },
];

const packSteps = [
  { item: "T-shirt zwart", qty: "2x" },
  { item: "Hoodie grijs", qty: "1x" },
  { item: "Sticker set", qty: "1x" },
];

const easeOutCubic: [number, number, number, number] = [0.22, 1, 0.36, 1];

const PackingControlAnimation = () => {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      packSteps.slice(1).forEach((_, index) => {
        timeoutsRef.current.push(
          window.setTimeout(() => {
            const nextIndex = index + 1;
            setPackIndex(nextIndex);
            if (nextIndex === packSteps.length - 1) {
      ...
      timeoutsRef.current.push(
        window.setTimeout(() => {
          setShowDetail(false);
        }, detailStart + stepDuration * (packSteps.length - 1) + pauseDuration)
      );

      timeoutsRef.current.push(
        window.setTimeout(() => {
          schedule();
        }, detailStart + stepDuration * (packSteps.length - 1) + pauseDuration + backDuration)
      );
    };
        ...
        >
          ...
          <div className="mt-4 space-y-3">
            {packSteps.map((step, index) => {
              const isDone = packIndex > index;
              const isActive = packIndex === index;
              ...
              return (
                ...
                >
                  <div className="flex flex-col">
                    <span className="text-xs inter-normal text-[#475569]">Artikel</span>
                    <span className="text-sm inter-semibold text-[#0F172A]">
                      {step.item}
                    </span>
                  </div>
...
export default PackingControlAnimation;
