...
const products = [
  { sku: "PR-204", name: "T-shirt zwart", stock: 86, color: "#16a34a" },
  { sku: "PR-205", name: "Hoodie grijs", stock: 42, color: "#1A5EE5" },
  { sku: "PR-206", name: "Pet blauw", stock: 24, color: "#f97316" },
];

const locations = [
  { label: "Locatie A-12", base: 18 },
  { label: "Locatie B-07", base: 12 },
  { label: "Locatie C-03", base: 12 },
];

const easeOutCubic: [number, number, number, number] = [0.22, 1, 0.36, 1];

const InventoryOverviewAnimation = () => {
  ...
  const [stockShift, setStockShift] = useState(false);
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      const updateDelay = 900;
      const pauseDuration = 3400;
      ...
      timeoutsRef.current.push(
        window.setTimeout(() => {
          setStockShift(true);
        }, detailStart + updateDelay)
      );

      timeoutsRef.current.push(
        window.setTimeout(() => {
          setShowDetail(false);
        }, detailStart + updateDelay + pauseDuration)
      );

      timeoutsRef.current.push(
        window.setTimeout(() => {
          schedule();
        }, detailStart + updateDelay + pauseDuration + backDuration)
      );
    };
          ...
          <div className="mt-5 space-y-3">
            {products.map((product, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-xs inter-normal text-[#475569]">
                  <span className="text-sm inter-semibold text-[#0F172A]">
                    {product.stock} stuks
                  </span>
                  <span
        ...
        >
          ...
          <div className="mt-4 space-y-3">
            {locations.map((location) => (
              <div
                key={location.label}
                className="flex items-center justify-between rounded-[14px] px-4 py-3 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.08)]"
              >
                <span className="text-sm inter-normal text-[#475569]">{location.label}</span>
                <motion.span
                  className="text-sm inter-semibold text-[#0F172A]"
                  animate={{ opacity: stockShift ? 0.6 : 1 }}
                  transition={{ duration: 0.4, ease: easeOutCubic }}
                >
                  {stockShift ? location.base - 2 : location.base} stuks
                </motion.span>
              </div>
            ))}
          </div>
          <div className="mt-3 text-xs inter-normal text-[#475569]">
            Update: voorraad is realtime gesynchroniseerd
          </div>
        </div>
      </motion.div>
...
export default InventoryOverviewAnimation;
