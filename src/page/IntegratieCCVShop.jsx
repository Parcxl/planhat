            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 w-full h-full items-center px-6 sm:px-10 lg:px-20">
                <Flex className="text-white w-full flex-col space-y-4 items-start text-left justify-center">
                    <p className="inter-semibold lg:text-[2rem] sm:text-[1.8rem] text-[1.5rem] text-left">
                        Het verzendplatform voor CCV Shop
                    </p>
                    <p className="text-left md:w-[90%] lg:mx-0 inter-medium lg:pl-0 pl-2 lg:text-[1.2rem]">
                        Verbind je CCV Shop-webshop en verzend sneller, goedkoper en zonder gedoe.
                    </p>
                    <Link
        ...
        <Flex className="lg:flex-row flex-col lg:items-center lg:space-x-12 lg:space-y-0 space-y-8 lg:w-[80%] w-[95%] mx-auto lg:pt-10 md:pt-8 sm:pt-6 pt-4">
            <Flex className="lg:w-[55%] w-[100%] flex-col space-y-6">
                <h2 className="inter-medium lg:text-[3.1rem] md:text-[2.7rem] sm:text-[2.3rem] text-[1.9rem] lg:leading-[3.6rem] md:leading-[3.1rem] sm:leading-[2.7rem] leading-[2.3rem] text-gray-900">
                    Gebruik <span className="bg-gradient-to-r from-[#1a5ee5] to-[#3b82f6] bg-clip-text text-transparent font-semibold">Sendwise</span> met CCV Shop
                </h2>
                <div className="flex flex-col space-y-6 inter-medium lg:text-[1.15rem] md:text-[1.08rem] text-[0.98rem] leading-[1.7rem] text-gray-600 lg:max-w-[34rem] w-full">
                    <p className="text-gray-700">
                        Gebruik Sendwise als centrale verzendlaag bovenop je CCV Shop-webshop. Orders uit CCV Shop komen automatisch binnen en staan direct klaar in je verzenddashboard. Je hoeft niets handmatig te exporteren of bij te houden — alles blijft netjes gesynchroniseerd. Zo blijft je fulfilmentproces strak en overzichtelijk.
                    </p>
                    <p className="text-gray-700">
                        Vanuit Sendwise selecteer je per order een vervoerder en maak je direct verzendlabels aan. De tracking-URL en barcode worden automatisch teruggekoppeld naar CCV Shop.
                    </p>
                </div>
                <Flex className="pt-2">
                    <Link
    ...
    return (
        <Flex className="flex-col w-[100%] overflow-hidden mb-10 pt-24 sm:pt-28 space-y-6 sm:space-y-8 lg:space-y-10">
            ...
            <IntegrationStepSection
                step={1}
                title="Stap 1 — Installeer de CCV Shop API-app"
                text="Ga naar de CCV Shop App Store en installeer de API-app. Deze app zorgt ervoor dat API-instellingen beschikbaar worden binnen je CCV Shop-omgeving, zodat Sendwise verbinding kan maken met je webshop."
                placeholder="Screenshot placeholder (CCV Shop App Store)."
                imageSrc="/ccv-step-1.png"
                imageAlt="CCV Shop App Store"
            />
            <IntegrationStepSection
                step={2}
                reverse
                title="Stap 2 — Maak API-gegevens aan in CCV Shop"
                text="Ga in CCV Shop naar Instellingen → Algemeen → API-gegevens. Klik op Nieuwe API-key, geef deze een herkenbare naam (bijvoorbeeld “Sendwise”) en klik op Opslaan. Klik vervolgens op Toepassen om de API-gegevens actief te maken. Je ontvangt nu een API key en API secret."
                placeholder="Screenshot placeholder (CCV Shop API-instellingen)."
                imageSrc="/ccv-step-2.png"
                imageAlt="CCV Shop API-instellingen"
            />
            <IntegrationStepSection
                step={3}
                title="Stap 3 — Vul de gegevens in bij Sendwise"
                text="Ga in Sendwise naar Integraties → CCV Shop. Vul hier het domein van je webshop in en plak de API key en API secret uit CCV Shop."
                placeholder="Screenshot placeholder (Sendwise CCV Shop integratieformulier)."
                imageSrc="/ccv-step-3.png"
                imageAlt="Sendwise CCV Shop integratieformulier"
            />
            <IntegrationStepSection
                step={4}
                reverse
                title="Stap 4 — Test en activeer de integratie"
                text="Klik op Test verbinding om te controleren of alles correct is ingesteld. Is de test succesvol? Klik dan op Integratie opslaan. Vanaf dat moment worden nieuwe CCV Shop-orders automatisch in Sendwise geladen."
                placeholder="Screenshot placeholder (Sendwise orders overzicht)."
                imageSrc="/ccv-step-4.png"
...
export default IntegratieCCVShop;
